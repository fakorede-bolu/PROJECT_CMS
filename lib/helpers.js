...
// Admin html helpers
helpers.adminHtml = (string, templateData, uniTempCallback) => {
    ...
    const getHeaderCallback = (error, headerString) => { //the error and headerString is received from the handlerCallback of the readTemplate helper when it was called with the header templateName
        if (!error && headerString) {
            // Get the footer
            helpers.readTemplate('Admin/_footer', templateData, (error, footerString) => {
    ...
    // Get the header
    helpers.readTemplate('Admin/_header', templateData, getHeaderCallback)

}


// Move the uploaded files to the public directory
helpers.uploadFile = (file, callback) => {
...
helpers.readTemplate = (templateName, templateData, indexCallback) => { 
    // check for the templatename type and validate
    templateName = typeof templateName === 'string' && templateName.length > 0 ? templateName : false;
