...
handlers._post.delete = (options, callback) => {
    ...
    if (options.reqMethod === 'delete' && id) {
       ...
       const data =  { _id: ObjectId(id) };
        ...
        lib.read("Posts", data, (err, post) => {
            if(!err && post) {    
                // get the comments associated to the post iindividually to be deleted.
                if (post[0].comments.length > 0) {
                    post[0].comments.map(element => {
                        const commentId = element.toString()
                        // get the comments and make sure it contains the comments specified
                        lib.read("Comments", { _id: ObjectId(commentId) }, (error, comments) => {
                            if (!error && comments) {
                                lib.delete("Comments", { _id: ObjectId(commentId) }, (err) => {
                                    if (!err) {
                                        // delete the post
                                        lib.delete("Posts", data, (err) => {
                                            if (!err) {
                                                callback(200, { "Done": "Post deleted" })
                                            } else {
                                                callback(500, { "Error": "Could not delete specified Post" })
                                            }
                                        })
                                    } else {
                                        callback(500, { "Error": "Could not delete specified Post Comments" })
                                    }
                    ...
                    lib.delete("Posts", data, (err) => {
                        if (!err) {
                            callback(200, { "Done": "Post deleted without comment" })
                        } else {
                            callback(500, { "Error": "Could not delete specified Post" })
                        }
                ...
                callback(500, { 'Error': 'Could not get specified Post' })
            }
        });
...
// All comment Handler - get
// Required Data - post id
handlers.allComments = (options, callback) => {
    // validate the required data
    const id = typeof (options.queryString.id) === 'string' && options.queryString.id.length === 24 ? options.queryString.id : false;

    if (id) {
        // read the post and grab the comments array
        lib.read("Posts", { _id : ObjectId(id) }, (error, post) => {
            if (!error && post) {
                // get the comment array and loop over to get all comments associated to it

                // Create an empty array to store all the comments that come back
                let commentArray = [];
               
                post[0].comments.map(eachComment => {

                    // read the comment associated to each of the post
                    const commentId = eachComment.toString()

                    lib.read("Comments", { _id: ObjectId(commentId) }, (error, comment) => {
                        if (!error && comment) {
                            commentArray.push(comment[0]);
                            // make sure you only callback when the loop is over.
                            if (commentArray.length === post[0].comments.length) {
                                callback(200, commentArray, 'json')
                            } 
                        } else {
                            callback(500, { "Error": "Could not read the comments associated to the post" })
                        }
                    });
                })

            } else {
                callback(500, { "Error": "Could not read the specified post" })
            }
        });
    } else {
        callback(400, {"Error" : "Missing required field"})
